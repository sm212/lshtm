...
local h = `1'
mac shift
local options "Logodds Cut(real -1.921) Samex Saving(string)"
parse "`*'"
preserve
...
#delimit ;
graph true approx param if true>-5&approx>-4, symbol(.i) connect(ss)
`yline' `xline' `xlab' t1("`t1'")
`ylab'
title("llr curve is in yellow, approximation in red")
...
if "`logodds'"!="" {
di ""
di in gr "Back on original scale"
